...
int main(){
    ...
    int maxNum = 10;
    FILE *fp;
//read maxNum from text file
fp = fopen("maxNum.txt", "r");
fscanf(fp, "%s", maxNumRead);
maxNum = atoi(maxNumRead);
fclose(fp);
menu_display:
...
if (userMaxNum > 10 || userMaxNum < 2)
{
    ...
    maxNum = userMaxNum;
    char userNumWrite[3] = "";
    itoa(maxNum, userNumWrite, 10); //converts new maxNum to a string to be saved into text file
    //saves new maxNum to text file
    fp = fopen("maxNum.txt", "w+");
    fprintf(fp, "%s", userNumWrite);
    fclose(fp); 
